...
from pynta.utils import get_unique_sym_struct_index_clusters, get_unique_sym, get_unique_sym_structs, get_unique_sym_struct_indices, rdkit_mol_to_ase_atoms
...
def conformer_exploration(
                        desorbed: rmgpy.molecule,
                        ase_output: bool = True) -> List[Atoms]:
    """
    Generate conformers of a supplied molecule using a seeded EmbedMultipleConfs, convert those conformers to ase Atoms
    objects, optimize the structure of each conformer using BFGS (fmax=.05), and return the list of optimized Atoms objects.
    
    Args:
        molecule_in_smiles: SMILES representation of molecule in gas phase
        ase_output: if True function gives list of ASE Atoms objects, if False function gives RDKit mol object and conformer
        id list
    
    Outputs:
        conformers: list containing conformers as ASE Atoms objects
        molecule_w_Hs: RDKit mol object with conformers, with hydrogens attached
        cids: conformer id numbers for all conformers generated from EmbedMultipleConfs connected to molecule_w_Hs"""
    
    # ARC dictionary defining number of conformers we should generate based on number of heavy atoms
    num_conf_dict = {(0, 3): 75,
                    (4, 9): 500,
                    (10, 29): 1000,
                    (30, 59): 2500,
                    (60, 99): 5000,
    ...
    # Application of the ARC dictionary to our molecule
    number_heavy_atoms = rdmol.GetNumAtoms()
    for key, value in num_conf_dict.items():
        if number_heavy_atoms >= key[0] and number_heavy_atoms <= key[1]:
            numConfs = value
    
    
    # Running ETKDG to generate conformers
    #params = AllChem.ETKDGv3() 
    cids = Chem.EmbedMultipleConfs(rdmol, numConfs = numConfs, randomSeed = 1) #params = params)
    
    
    # Generating Atoms object for each conformer found & calculating potential energy
    conformers = []
    indmap_dict = {}
    dict_counter = 0
    for cid in cids:
        ...
        # Converting rdkit_mol to Atoms object with vacuum and pbc. Then run BFGS structure optimization on each atom.
        atom = rdkit_mol_to_ase_atoms(Chem.Mol(rdmol, confId=cid))
        conformers.append(atom)
        ...
        indmap = {i:rdmap[a] for i,a in enumerate(atom.atoms)}
        indmap_dict[dict_counter] = indmap
        dict_counter += 1
    
        
    if ase_output == True:
        return conformers, indmap_dict
    else:
        return rdmol, cids

def find_target_conformers(desorbed, energy_threshold = .05):
    """
    Enumerate through the conformer list that is generated by conformer_exploration to find conformers that are a minimum energy away from the accepted conformer list. 
    The first conformer in the initial conformer list is taken because it's unique, then every other conformer is compared to the accepted list to check if it's (energy_threshold) away
    from the conformers that are placed inside the target_conformers list.
    
    Args:
    desorbed: rmgpy molecule object
    energy_threshold: minimum energy difference (eV) that structurally significant conformers must have between them
    
    Output:
    target_conformers: list of structurally significant conformers
    indmap_dict: dictionary containing the indmap for each structurally significant conformer
    """
    
    conformer_list, indmap_dict = conformer_exploration(desorbed)
    
    # Setting up empty list and dict as well as loop_index to retrieve the correct indmap for each chosen conformer.
    target_conformers = [conformer_list[0]]
    atom_map_dict = {}
    loop_index = 0
    
    for atom in conformer_list:
        atom_index = conformer_list.index(atom)
        conformer_energy = atom.get_potential_energy()
        unique_conformer = []
        
        for accepted_conformer in target_conformers:
            if abs(conformer_energy - accepted_conformer.get_potential_energy()) > energy_threshold:
                unique_conformer.append("True")
            else:
                unique_conformer.append("False")
        
        if "False" not in unique_conformer:
            target_conformers.append(atom)
            atom_map_dict[loop_index] = indmap_dict[atom_index]
            
        loop_index += 1
    
    return target_conformers, atom_map_dict
